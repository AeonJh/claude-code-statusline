...
if [[ "$model" != *context* && "$model" != *Context* ]]; then
  if (( ctx_size_int >= 1000000 )); then
    ctx_label=" ${GRAY}$((ctx_size_int / 1000000))M${RST}"
  elif (( ctx_size_int >= 1000 )); then
    ctx_label=" ${GRAY}$((ctx_size_int / 1000))k${RST}"
  fi
fi
